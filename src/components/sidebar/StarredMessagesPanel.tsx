...
import { ArrowLeft, Search, Star, MessageSquare, Image as ImageIcon, Video, File } from "lucide-react";
import { useQuery } from "convex/react";
import { api } from "../../../convex/_generated/api";
import { useAuthStore } from "@/store/authStore";
import { useChatStore } from "@/store/chatStore";
import { Id } from "../../../convex/_generated/dataModel";
import { decryptMessage } from "@/crypto/encryption"; // ── FIX: For Decryption ──
import { base64ToKey } from "@/crypto/keyDerivation"; // ── FIX: For Decryption ──

interface StarredMessagesPanelProps {
  onBack: () => void;
}

export default function StarredMessagesPanel({ onBack }: StarredMessagesPanelProps) {
  const userId = useAuthStore((s) => s.userId);
  const secretKey = useAuthStore((s) => s.secretKey); // ── FIX: Get Secret Key for decryption ──
  ...
  const conversations = useQuery(
    api.conversations.getConversationsList, // ── FIX: Changed to getConversationsList ──
    userId ? { userId: userId as Id<"users"> } : "skip"
  );

  // Theme support (SlimBar style)
  ...
  const formattedMessages = (starredMessages || []).map((msg) => {
    const conv = conversations?.find((c: any) => c.conversationId === msg.conversationId);
    ...
    if (conv) {
      ...
      if (msg.type === "text" && secretKey && conv.publicKey && msg.encryptedContent && msg.iv) {
        try {
          const theirPublicKey = base64ToKey(conv.publicKey);
          text = decryptMessage({ encryptedContent: msg.encryptedContent, iv: msg.iv }, secretKey, theirPublicKey);
        } catch (e) {
      ...
      // 2. Create correct object for setActiveChat
      properActiveChatObj = {
        ...
        publicKey: conv.publicKey, // ── FIX: Removed invalid colors, kept publicKey ──
      };
    }
  ...
  const handleMessageClick = (convObj: any, messageId: string) => {
    if (convObj) {
      setActiveChat(convObj); // ── FIX: Pass the whole conversation object instead of string ──
    }
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-4 custom-scrollbar">
        {starredMessages === undefined ? (
          ...
          <div className="flex flex-col items-center justify-center h-full opacity-60 mt-10">
            ...
            <p className="text-[14px] font-medium text-foreground">No starred messages</p>
          ...
          <div className="space-y-1.5">
            {filteredMessages.map((msg) => (
              <button
                key={msg.id}
                onClick={() => handleMessageClick(msg.properActiveChatObj, msg.id)}
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-2 min-w-0">
                    <MessageSquare size={14} className="text-primary opacity-80 shrink-0" />
                ...
                <div className="text-[13px] text-muted-foreground line-clamp-2 leading-relaxed ml-5">
                  {msg.type === "text" ? (
                    ...
                    <span className="flex items-center gap-1.5"><File size={14} /> Document</span>
                  )}
                </div>
              </button>
            ))}
