// src/components/chat/media/MediaGridReactBar.tsx
...
interface MediaGridReactBarProps {
  onEmojiSelect: (emoji: string) => Promise<void>;
}

export default function MediaGridReactBar({
  onEmojiSelect,
}: MediaGridReactBarProps) {
  ...
  return (
    <>
      <div
        className={`relative self-center transition-opacity ${showQuickReact || showEmojiPicker ? "opacity-100" : "opacity-0 group-hover/grid:opacity-100"}`}
      >
        <button
          ref={smileButtonRef}
          onClick={() => {
            if (showQuickReact) setShowQuickReact(false);
            else openQuickReact();
          }}
